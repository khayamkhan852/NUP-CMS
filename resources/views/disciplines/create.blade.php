            ...
            <div class="col-lg-12">
                <div class="ibox ">
                    <div class="ibox-title">
                        <h5>Create New Discipline</h5>
                        <div class="ibox-tools">
                        </div>
                    </div>
                    <div class="ibox-content">
                        <form action="{{ route('settings.disciplines.store') }}" method="post">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <x-text-input id="code" class="{{ $errors->has('code') ? 'is-invalid' : '' }}"
                                                  name="code" value="{{ old('code')}}" autofocus placeholder="Discipline Code..." autocomplete="off" />
                                    @error('code')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <x-text-input id="name" class="{{ $errors->has('name') ? 'is-invalid' : '' }}"
                                                  name="name" value="{{ old('name')}}" placeholder="Discipline Name..." autocomplete="off" />
                                    @error('name')
                                    <x-input-error>{{ $message }}</x-input-error>
                                    @enderror
                                </div>
                            ...
                            <button type="submit" class="btn btn-primary mt-3"><i class="fa fa-save"></i> Submit New Discipline</button>
                        </form>
                    </div>
                </div>
            </div>
